...
export class createRelationCategoryProduct1653413529362
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `product` ADD `created_at` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6)'
    );
    await queryRunner.query('ALTER TABLE `product` DROP COLUMN `updated_at`');
    await queryRunner.query(
      'ALTER TABLE `product` ADD `updated_at` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6)'
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query('ALTER TABLE `product` DROP COLUMN `updated_at`');
    await queryRunner.query(
      'ALTER TABLE `product` ADD `updated_at` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP'
    );
    await queryRunner.query('ALTER TABLE `product` DROP COLUMN `created_at`');
    await queryRunner.query(
      'ALTER TABLE `product` ADD `created_at` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP'
    );
